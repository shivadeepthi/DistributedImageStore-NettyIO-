...
public class HeartbeatManager extends Thread {
	...
	public void addOutgoingChannel(int nodeId, String host, int mgmtport, Channel ch, SocketAddress sa) {
		if (!outgoingHB.containsKey(ch)) {
			...
			/*@Minu
			 * adding the active status nodes to the sharding manager
			 */
			if(!ShardingManager.getInstance().getServerIds().contains(nodeId)) {
				logger.info("hashing and adding the node : "+nodeId+" to the connectedNodes!!!!!!!!!");
				
				ShardingManager.getInstance().hashAndAdd(nodeId);
			}
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				// check if a node goes down whether it is leader node
				Integer leaderId = ElectionManager.getInstance().whoIsTheLeader();
				if(heart.getNodeId() == leaderId) 
					ElectionManager.getInstance().startElection();
				if(ShardingManager.getInstance().checkExistence(heart.getNodeId()))
					ShardingManager.getInstance().removeFailedNode(heart.getNodeId());
