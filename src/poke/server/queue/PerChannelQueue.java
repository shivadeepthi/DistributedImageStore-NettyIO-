...
public class PerChannelQueue implements ChannelQueue {
	...
	public void shutdown(boolean hard) {
		...
		if (iworker != null) {
			logger.info("setting forever as false in iworker----- > in shut down");
		...
		if (oworker != null) {
			logger.info("setting forever as false in oworker----- > in shut down");
				...
				try {
					...
					if (msg instanceof Request) {
						...
						if (channel != null && channel.isOpen() && channel.isWritable()) {
							logger.info("@Minu---->reaching in oworker- ready to flush");
	...
	protected class InboundWorker extends Thread {
		...
		public void run() {
			...
			while (true) {
				...
				{
					logger.info("the message picked from queue is zero ");
					break;
				}
				...
				try {
					...
					if (msg instanceof Request) {
						...
						logger.info("@Minu-->request -iworker : "+req.getHeader());
...
//							long millis = 0;
//							String jobId = null;
//							if(!req.getBody().getJobOp().getData().hasJobId()) {
//							if(req.getBody().getJobOp().getData().getJobId() == null || req.getBody().getJobOp().getData().getJobId().equals(""))
//							{
//								logger.info("Not getting the JobId");
//							millis = System.currentTimeMillis() % 1000;
//							jobId = String.valueOf(millis);
//							req = ResourceUtil.buildMessageNewWithJobId(req, jobId);
//							logger.info("job Id us : "+jobId);
//						}
//						}
//						}
						...
						{
							// based on header take the request
	...
	public class CloseListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			logger.info("@Minu -> calling operation Complete -PerChannel");
			sq.shutdown(true);
		}
	}
}
